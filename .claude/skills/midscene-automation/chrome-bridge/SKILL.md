...
```bash
npx @midscene/web@1 connect --bridge --url https://example.com
```

Or just connect to the current page without specifying the URL:

```bash
npx @midscene/web@1 connect --bridge
```
...
2. **Execute action**: Use `act` to perform the operation.
3. **Disconnect**: Disconnect when done (optional).

## Best Practices

1. **Be specific about UI elements**: Instead of `"the button"`, say `"the blue Submit button in the contact form"`.
2. **Use natural language**: Describe what you see on the page, not CSS selectors. Say `"the red Buy Now button"` instead of `"#buy-btn"`.
3. **Handle loading states**: After navigation or actions that trigger page loads, take a screenshot to verify the page has loaded.
4. **Disconnect when done**: Always disconnect to free resources.
...
When the command fails to run, check `.env` in the current working directory (Midscene loads it automatically). Midscene requires models with strong visual grounding. Configure these environment variables:
...
Commonly used models: Doubao Seed 1.6, Qwen3-VL, Zhipu GLM-4.6V, Gemini-3-Pro, Gemini-3-Flash. See [Model Configuration](https://midscenejs.com/model-common-config) for supported providers.
...
### Timeouts
- Web pages may take time to load. Take a screenshot to verify readiness before interacting.
